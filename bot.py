...
from config import API_HASH, APP_ID, LOGGER, TG_BOT_TOKEN, TG_BOT_WORKERS, FORCE_SUB_CHANNEL1, FORCE_SUB_CHANNEL2, FORCE_SUB_CHANNEL3, FORCE_SUB_CHANNEL4, CHANNEL_ID
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL4:
            try:
                ...
                self.LOGGER(__name__).warning(a)
                self.LOGGER(__name__).warning("Bot can't Export Invite link from Force Sub Channel!")
                self.LOGGER(__name__).warning(f"Please Double check the FORCE_SUB_CHANNEL3 value and Make sure Bot is Admin in channel with Invite Users via Link Permission, Current Force Sub Channel Value: {FORCE_SUB_CHANNEL4}")
                self.LOGGER(__name__).info("\nBot Stopped. Join https://t.me/OuraaSupport for support")
                sys.exit()
        try:
        ...
        self.LOGGER(__name__).info(f"Bot Running..!\n\nCreated by Lord Ouraa\nhttps://t.me/Owaitingforyou")
